...
| 3 | 源覆盖 + 训练对接 | 7 | **in_progress** (W3-1 next) | 4 种格式跑通 + HF 导出 |
...
| adapter-raw-upload-20260520 | W3-1 | pending | — | — | — | — |
...
- **active change**: `adapter-raw-upload-20260520` (W3-1)
- **next action**: application-owner 自写 mini-design（Bronze 层 raw-upload adapter：HTTP multipart → BlobStore CAS）→ sonnet 端到端 → opus verify
- **Wave 1 + Wave 2 Checkpoint 状态**：全部 done（4/4 + 6/6）；recipe v2 + silver snapshot 持久化整链跑通。
- **Wave 3 启动**：源覆盖优先（adapter / loader 7 个 change），W3-7 依赖 W2-6 已解；W3-1..W3-5 可独立串行。
...
- Changes done: **10 / 27**（W1-1 a51a126, W1-2 af8a7d5, W1-3 13abd0d, W1-4 999ca82, W2-1 5d0ed99, W2-2 0f8f6ba, W2-3 c288082, W2-4 4dd1d3e, W2-5 153c2c9, W2-6 ac82567）
- Wave 1 全部 done（4/4）；Wave 2 全部 done（6/6）
- Phase 1 reviewer cycles: 1（W1-1；W1-2 起 v3 不再跑）
- Phase 3 reviewer cycles: 10（W1-1..W2-6 全部 APPROVED）
- v3 mini-design 实测：W1-2..W2-6 design 56-140 行 / 3-4 AC；application-owner + sonnet + opus 各 1 次 spawn，verify ~2-7 min
- W1-4 / W2-1 / W2-2 / W2-3 / W2-4 / W2-5 / W2-6 都是 APPROVED（W2-4 含 2 个 ACCEPT AS-IS deviation；其余 0 偏离）
- packages/core 测试：62/62 PASS（recipe v2 + dataset export + 全部 W2-* 算子）
- BIG REWRITE 次数: 0
- MAJOR ISSUE 次数: 0
