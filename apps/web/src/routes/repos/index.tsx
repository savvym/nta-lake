...
function ReposListPage() {
  ...
  if (data.items.length === 0) return <div className="text-gray-500">暂无 repository</div>;
            ...
            <Card className="hover:shadow-md transition-shadow">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${LAYER_COLORS[r.layer] ?? "bg-gray-100 text-gray-700"}`}>
                    {r.layer}
                  </span>
                </CardTitle>
              </CardHeader>
